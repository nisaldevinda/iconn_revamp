...
import { downloadSalaryIncrementTemplate, uploadSalaryIncrementSheet, completeSalaryIncrementProcess, getSalaryIncrementUploadHistory, rollbackSalaryIncrementUpload } from '@/services/bulkUpload';
...
const SalaryIncrementBulkUpload: React.FC = () => {
  ...
  const downloadTemplate = async () => {
    ...
    downloadSalaryIncrementTemplate().then((response: APIResponse) => {
      if (response.error) {
        message.error({
          content:
            response.message ??
            ...
            renderItem={(item, index) => (
              ...
              >
                <List.Item.Meta
                  ...
                  description={`${JSON.parse(item.employeeSalaryIds).length} number of records has been created`}
                />
              </List.Item>
            )}
          />
