...
const EmployeeStatusCard: React.FC<EmployeeStatusCardProps> = (props) => {
  const intl = useIntl();
  ...
  useEffect(() => {
    setJobs(
      (props.employee?.jobs ?? []).map((job: any) => {
        return {
          ...job,
          previousRecord: (props.employee?.jobs ?? []).find(
            (_job: any) => _job.id == job.previousRecordId,
          ),
        };
      }),
    );

    if (!_.isEmpty(props.employee)) {
      getProfilePicture(props.employee.id);
    }
  }, [props.employee]);
  ...
  const onChangeHistoryModalSelctor = (value) => {
    ...
    const _records =
      value == 'ALL' ? jobs : jobs?.filter((job) => job.employeeJourneyType == value);
          ...
          <Button
            ...
            icon={
              <Icon
                ...
                component={() => <img src={HistoryIcon} height={15} width={15} />}
              />
            }
            onClick={() => {
      ...
      >
        <Row>
                ...
                <Typography.Text className="colon-before-text">
                  {
                    props.data?.departments?.find(
                      (record) => record.value == props.currentJob?.departmentId,
                    )?.label
                  }
                </Typography.Text>
              </Col>
                ...
                <Typography.Text className="colon-before-text">
                  {
                    props.data?.payGrades?.find(
                      (record) => record.value == props.currentJob?.payGradeId,
                    )?.label
                  }
                </Typography.Text>
              </Col>
                ...
                <Typography.Text className="colon-before-text">
                  {
                    props.data?.jobCategories?.find(
                      (record) => record.value == props.currentJob?.jobCategoryId,
                    )?.label
                  }
                </Typography.Text>
              </Col>
          ...
          {/* <Col span={8}>
            <Row>
              <Col span={12}>
                <FormattedMessage id="employee_journey_update.division" defaultMessage="Division" />
              </Col>
              <Col span={12}>
                <Typography.Text className="colon-before-text">
                  {
                    props.data?.divisions?.find(
                      (record) => record.value == props.currentJob?.divisionId,
                    )?.label
                  }
                </Typography.Text>
              </Col>
                ...
                <Typography.Text className="colon-before-text">
                  {
                    props.data?.jobTitles?.find(
                      (record) => record.value == props.currentJob?.jobTitleId,
                    )?.label
                  }
                </Typography.Text>
              </Col>
                ...
                <Typography.Text className="colon-before-text">
                  {
                    props.data?.locations?.find(
                      (record) => record.value == props.currentJob?.locationId,
                    )?.label
                  }
                </Typography.Text>
              </Col>
          ...
          <Col span={8}>
            <Row>
              ...
              <Col span={12}>
                <Typography.Text className="colon-before-text">
                  {
                    props.data?.managers?.find(
                      (record) => record.value == props.currentJob?.reportsToEmployeeId,
                    )?.label
                  }
                </Typography.Text>
              </Col>
            ...
            <Row>
              ...
              <Col span={12}>
                <Typography.Text className="colon-before-text">
                  {
                    props.data?.managers?.find(
                      (record) => record.value == props.currentJob?.functionalReportsToEmployeeId,
                    )?.label
                  }
                </Typography.Text>
              </Col>
                ...
                <Typography.Text className="colon-before-text">
                  {
                    props.data?.calendars?.find(
                      (record) => record.value == props.currentJob?.calendarId,
                    )?.label
                  }
                </Typography.Text>
              </Col>
          ...
          {/* Calendar */}
          {props.mode == 'confirmation_contracts' && (
            <Col span={8}>
              <Row>
                <Col span={12}>
                  <FormattedMessage
