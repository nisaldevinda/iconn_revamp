...
import {
  ...
  addWorkflowApproverPool,
  updateWorkflowApproverPool,
  removeWorkflowApproverPool,
} from '@/services/workflowServices';
import { genarateEmptyValuesObject } from '@/utils/utils';
import CreateForm from './createPool';
...
const WorkflowApproverPool: React.FC = () => {
  ...
  useEffect(() => {
    if (!model) {
      getModel('workflowApproverPool').then((model) => {
        if (model && model.data) {
          setModel(model.data);
        }
      });
  ...
  const addViewProps = {
    title: intl.formatMessage({
      id: `add_emp_group`,
      defaultMessage: `Add Approver Pool`,
    }),
    key: `add_emp_group`,
    ...
    onFinish: async () => {
      ...
      let params = {
        id: currentRecord.id,
        poolName: currentRecord.poolName,
        poolPermittedEmployees: JSON.stringify(currentRecord.poolPermittedEmployees),
      };

      await addWorkflowApproverPool(params)
  ...
  const editViewProps = {
    title: intl.formatMessage({
      id: `edit_emp_grp`,
      defaultMessage: `Edit Approver Pool`,
    }),
    key: `edit_emp_grp`,
    ...
    onFinish: async () => {
      ...
      await updateWorkflowApproverPool(params)
        .then((response: APIResponse) => {
          if (response.error) {
            message.error({
              content:
  ...
  return (
    <>
      <BasicContainer
        ...
        refresh={refresh}
        model={model}
        tableColumns={[{ name: 'poolName', sortable: true, filterable: true }]}
        recordActions={['add', 'edit', 'delete']}
        searchFields={['poolName']}
        addFormType="function"
        editFormType="function"
        getAllFunction={workflowApproverPools}
        addFunction={async () => {
        ...
        deleteFunction={removeWorkflowApproverPool}
        permissions={{
