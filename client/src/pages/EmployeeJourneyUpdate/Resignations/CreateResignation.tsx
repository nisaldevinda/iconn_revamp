...
import { createResignation } from '@/services/employeeJourney';
...
interface CreateResignationProps {
    data: any,
    employee: any,
...
const CreateResignation: React.FC<CreateResignationProps> = (props) => {
    const intl = useIntl();
    ...
    const onFinish = async (data: any) => {
        ...
        createResignation(props.employee.id, requestData)
            .then(async (response) => {
    ...
    return (<>
        ...
        {props.hasUpcomingJobs && <Alert
                ...
                message={intl.formatMessage({
                    ...
                    defaultMessage: "User cannot create resignation until the upcoming resignation is done",
                })}
            />}
            <ProForm
                form={formRef}
                ...
                <Row gutter={12}>
                    ...
                    <ProFormDependency name={['effectiveDate', 'resignationHandoverDate']}>
                        {({ effectiveDate, resignationHandoverDate }) => {
                            ...
                            const noticeDayCount = moment.duration(effectiveDateMoment.diff(resignationHandoverDateMoment)).asDays();

                            let remainingDayCount = 0;
                            if (props.employee.noticePeriod) {
                                const noticePeriod = parseHumanReadableDurationToDayCount(props.employee.noticePeriod);
                                remainingDayCount = noticePeriod - noticeDayCount > 0 ? noticePeriod - noticeDayCount : 0;
                            }
