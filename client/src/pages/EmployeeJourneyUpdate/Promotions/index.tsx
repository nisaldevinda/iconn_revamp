                ...
                <Row gutter={10}>
                    <Col span={12}>
                        <CreateNewPromotion
                            ...
                            hasUpcomingJobs={!_.isEmpty(employee?.jobs?.filter(job => job.employeeJourneyType == 'PROMOTIONS' && !job.isRollback && job.effectiveDate > props?.data?.companyDate) ?? [])}
                        />
                    </Col>
                    <Col span={12}>
                        <EmployeeJourneyUpcoming
                            title={intl.formatMessage({
                                id: 'employee_journey_update.upcoming_promotions',
                                defaultMessage: "Upcoming Promotions",
                            })}
                            employee={employee}
                            setEmployee={setEmployee}
                            data={props.data}
                            records={employee?.jobs?.map((job: any) => {
                                ...
                                .filter(job => job.employeeJourneyType == 'PROMOTIONS' && job.effectiveDate > props?.data?.companyDate) ?? []}
                        />
                        <EmployeeJourneyHistory
                            title={intl.formatMessage({
                                id: 'employee_journey_update.promotion_history',
                                defaultMessage: "Promotion History",
                            })}
                            employee={employee}
                            data={props.data}
                            records={employee?.jobs?.map((job: any) => {
