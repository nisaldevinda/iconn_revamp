...
import ProForm, { ProFormDatePicker, ProFormDigit, ProFormField, ProFormRadio, ProFormSelect, ProFormText, ProFormTextArea } from '@ant-design/pro-form';
...
import { addLeaveEntitlement, addLeaveEntitlementForMultiple, getLeaveEntitlement, getLeaveType, getLeaveTypes } from '@/services/leaveEntitlment';
...
const LeaveEntitlement: React.FC = (props) => {
    ...
    useEffect(() => {
        if (selectedLeavePeriod) {
            setLeavePeriodStartDate(moment(leavePeriodArray[selectedLeavePeriod].leavePeriodStartDate, "DD-MM-YYYY").format("YYYY-MM-DD")
            )
            setLeavePeriodEndDate(moment(leavePeriodArray[selectedLeavePeriod].leavePeriodEndate, "DD-MM-YYYY").format("YYYY-MM-DD")
            )
            form.setFieldsValue({
    ...
    const generateTreeData = async () => {
        ...
        if (allEmployees) {
            ...
            if (departmentValue && locationValue) {
                filteredEmployees = _.filter(allEmployees, o => o.currentJobs.departmentId === departmentValue && o.currentJobs.locationId === locationValue)
            }
            else if (departmentValue && !locationValue) {
                filteredEmployees = _.filter(allEmployees, o => o.currentJobs.departmentId === departmentValue)

            }
            else if (!departmentValue && locationValue) {
                filteredEmployees = _.filter(allEmployees, o => o.currentJobs.locationId === locationValue)
            }
            else {
