...
import { addUserRoles, queryUserRolesMeta, getAccessManageFields, getAccessManageMandotaryFields } from '@/services/userRole';
...
const AccessLevels: React.FC = () => {
  ...
  // Role type data get from backend
  const getUserRoleMeta = async () => {
    try {
      ...
      setFieldAccessLevels(accessLevels);
  ...
  const handleOnRoleChange = (value: string) => {
    const permissionKeys = rolePermissions[value]['permission-list'];
    const scopeAccess = rolePermissions[value]['scopeAccess'] ?? [];
    const permissionLabels = getPermissionLabels(permissionKeys);
    const availableWorkflows = rolePermissions[value]['workflows'] ?? [];
    const workflowAccess = getWorkflowObjects(availableWorkflows);
    setSelectedRole(value);
    setpermittedActionsCheckList([]);
    setSelectedRolePermissions(permissionLabels);
    setScopeAccess(getScopes(scopeAccess));
    setWorkflowAccess(workflowAccess);
  };
    ...
    if (permissionLevel !== '') {
      ...
      selectedRowKeys.forEach((selectedKey: string) => {
        ...
        if (index !== -1) {
          ...
          fieldPermissions.splice(index, 1, { ...obj, permission: permissionLevel });
        }
      });
  ...
  const handleAdd = async () => {
    try {
      ...
      history.push(`/settings/accesslevels/editrole/${data.id}`);
          ...
          form.setFields([
            {
              name: fieldName,
              errors:
                error.data[fieldName][0] === 'This is an unique field.'
                  ? ['Role Name is already existing']
      ...
      <>
        <Row style={{ background: 'white' }}>
          <Form form={form} layout="vertical">
            ...
            >
              <Input placeholder="Add Role Name" />
            </Form.Item>
          </Form>
        </Row>
        ...
        <Row style={{ height: 'auto', background: 'white' }}>
          {selectedRole && selectedRolePermissions.length > 0 ? (
            <Col span={8}>
              ...
              <Checkbox.Group
                ...
                defaultValue={permittedActionsCheckList}
              >
          ...
          {selectedRole && !_.isEmpty(scopeAccess) ? (
            <Col span={8}>
              ...
              <Checkbox.Group style={{ width: '100%' }} onChange={handleOnChangeScope}>
                  ...
                  return selectedRole === 'MANAGER' ? (
                    <>
                      {scope.scopeDataList.map((actinPremission: any) => {
                        return (
                          <Row key={actinPremission.key}>
                            <Checkbox data-key={actinPremission.key} value={actinPremission.key}>{actinPremission.value}</Checkbox>
                          </Row>
                        );
                      })}
                    </>
        ...
        <Row style={{ background: 'white', float: 'right' }}>
          <Row style={{ marginTop: '2vh', float: 'right', background: 'white' }}>
            <FooterToolbar>
              <Button data-key="back" style={{ marginRight: '1vh' }} onClick={handleCancel}>
                {intl.formatMessage({
                  id: 'BACK',
                  defaultMessage: 'Back',
                })}
              </Button>
              <Button data-key="submit" type="primary" style={{ marginRight: '1vh' }} onClick={handleAdd}>
                {intl.formatMessage({
                  id: 'SUBMIT',
