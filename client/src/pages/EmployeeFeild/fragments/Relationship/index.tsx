...
const Relationship: React.FC = () => {
    ...
    useEffect(() => {
        if (!model) {
            getModel(Models.Relationship).then((model) => {
                if (model && model.data) {
                    setModel(model.data)
                }
            })
    ...
    return (
      <BasicContainer
        ...
        addFormType="model"
        editFormType="drawer"
        getAllFunction={getAllRelationships}
        addFunction={addRelationship}
        editFunction={updateRelationship}
        deleteFunction={removeRelationship}
        permissions={{
