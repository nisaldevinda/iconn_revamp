...
import BalanceIcon from '../../../assets/leave/icon-circle-green.svg';
import '../style.css';
...
const AssignLeave: React.FC = () => {
  ...
  const [relatedEmployeesWithInactiveEmps, setRelatedEmployeesWithInactiveEmps] = useState([]);
  const [isEnableInactiveEmployees, setIsEnableInactiveEmployees] = useState<any>(false);
  const [allEmployeeList, setAllEmployeeList] = useState<Array<any>>([]);
  ...
  const getShiftDataForLeaveDate = async (periodType: any) => {
    if (selectedEmployee) {
      ...
      if (periodType == 'IN_SHORT_LEAVE' && res.data.shift != null && fromDate) {
        ...
        form.setFieldsValue({
          fromTime: moment(res.data.shift.startTime, 'HH:mm'),
          toTime: toTime,
        });
        setToTime(time);
      ...
      if (periodType == 'OUT_SHORT_LEAVE' && res.data.shift != null && fromDate) {
        ...
        form.setFieldsValue({
          toTime: moment(res.data.shift.endTime, 'HH:mm'),
          fromTime: fromTime,
        });
  ...
  const calculateWorkingDaysCount = async () => {
    if (selectedleaveType && fromDate && toDate) {
      const res = await calculateWorkingDaysCountForLeaveAssign(
        selectedleaveType,
        fromDate,
        toDate,
        selectedEmployee,
  ...
  const getRelatedEmployees = async () => {
    try {
      const actions: any = [];
      const inactiveActions: any = [];
      // const { data } = await getEmployeeList('ADMIN');
      const { data } = await getAllEmployeeList('ADMIN');


      const res = data.map((employee: any) => {
        inactiveActions.push({ value: employee.id, label: employee.employeeNumber+' | '+employee.employeeName });
      ...
      setRelatedEmployeesWithInactiveEmps(inactiveActions);
      return res;
    } catch (err) {
      console.log(err);
      return [];
  ...
  return (
    <>
                    ...
                    <Row>
                      ...
                      >
                        ...
                        >
                          {intl.formatMessage({
                            id: 'includeInactive',
                            defaultMessage: 'Enable For Inactive Employees',
                          })}
                        </Text>
                        <Switch
                        ...
                        <Row>
                          <Col md={{ span: 10 }} lg={{ span: 7 }} xl={{ span: 7 }}>
                            <Form.Item
                              ...
                              name="fromDate"
                              label={isAllowShowFromDateRowOnly ? 'Date' : 'From Date'}
                              rules={[
                                {
                                  required: true,
                                  message: 'Required',
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                ...
                                {selectedleaveTypeObject?.halfDayAllowed ? (
                                  <Radio value={3}>Second Half</Radio>
                                ) : null}
                                ...
                                {Number(workingDaysCount) > 1 ? (
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      {selectedleaveTypeObject?.halfDayAllowed ? (
                                        <Radio value={2}>First Half</Radio>
                                      ) : null}
                                      {selectedleaveTypeObject?.halfDayAllowed ? (
                                        <Radio
      ...
      >
        <LeaveHistoryList selectedEmployee={selectedEmployee} listType="leave"></LeaveHistoryList>
      </ModalForm>
    </>
  );
};
