...
import {
  getEmployeeShortLeaveDataSet,
  getAdminShortLeaveDataSet,
  getAdminLeaveHistoryDataSet,
  cancelAdminAssignLeave,
  cancelAdminAssignShortLeave
} from '@/services/leave';
...
const LeaveHistoryList: React.FC<TableViewProps> = (props) => {
  ...
  async function callGetLeaveRequestData(
    ...
    try {
      ...
      if (props.listType == 'shortLeave') {
        ...
        getShortLeaveStates();
        await getAdminShortLeaveDataSet(params).then((response: any) => {
          if (response) {
        ...
        await getAdminLeaveHistoryDataSet(params).then((response: any) => {
          if (response) {
            setLeaveRequestData(response.data.sheets);
            setDataCount(response.data.count);
          }
  ...
  const cancelAssignLeaveRequest = (leaveRequestId: any) => {
    try {
      cancelAdminAssignLeave(leaveRequestId).then((res) => {
      ...
      if (leaveData.workflowInstanceIdNo) {
        ...
        if (result['data'] !== null) {
          ...
          Object.keys(result['data']).some((key) => {
            result['data'][key]['levelName'] = key == 'Intial Level' ? key :  key + ' Approval';
  ...
  const cancelAssignShortLeaveRequest = (leaveRequestId: any) => {
    try {
      cancelAdminAssignShortLeave(leaveRequestId).then((res) => {
        // setIsModalVisible(false);
        // let sortValue = { name: 'fromDate', order: 'DESC' };
        // callGetLeaveRequestData(1, 20, sortValue);
        actionRef.current?.reload();
  ...
  const shortLeaveColumns: ProColumns<LeaveRequestItem>[] = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <>
          {
            record.canDirectCancelByAdmin ? 
            <Space>
              <Popconfirm
                title="Are you sure?"
                placement="top"
                onConfirm={() => {
                  cancelAssignShortLeaveRequest(record.id);
                }}
                okText="Yes"
                cancelText="No"
              >  
                <Tooltip placement="bottom" title={'Cancel Short Leave'}>
                  <a href="" style={{fontSize: 15}}><CloseCircleOutlined /></a>
                </Tooltip>
              </Popconfirm>
            </Space> : <></>
          }
        ...
        <>
          {
            ...
            <Space>
              <Popconfirm
                ...
                placement="top"
                onConfirm={() => {
                  cancelAssignLeaveRequest(record.id);
                }}
                okText="Yes"
                cancelText="No"
              >  
                <Tooltip placement="bottom" title={'Cancel Leave'}>
                  <a href="" style={{fontSize: 15}}><CloseCircleOutlined /></a>
                </Tooltip>
              </Popconfirm>
            </Space> : <></>
