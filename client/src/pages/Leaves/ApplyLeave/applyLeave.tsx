...
import { getEmployeeListForCoveringPerson } from '@/services/dropdown';
...
const ApplyLeave: React.FC = () => {
  ...
  const [routeState, setRouteState] = useState<any>(undefined);
  const [leaveReason, setLeaveReason] = useState<string | null>(null);
  const [selectedCoveringPerson, setSelectedCoveringPerson] = useState<string | null>(null);
  ...
  // const [workingDaysCountWithinDateRange, setWorkingDaysCountWithinDateRange] = useState<String | null>();
  ...
  const [canShowCoveringPerson, setCanShowCoveringPerson] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (routeState) {
      ...
      if (routeState.date) {
        setFromDate(routeState.date);
        setToDate(routeState.date);
        form.setFieldsValue({ fromDate: moment(routeState.date, 'YYYY-MM-DD'), toDate: moment(routeState.date, 'YYYY-MM-DD') });
      }
    } else {
      setFromDate(null);
      setToDate(null);
    ...
    try {
      ...
      if (fileFormatError) {
        ...
        const params: any = {
          ...
          manageCoveringPerson: canShowCoveringPerson,
          selectedCoveringPerson: selectedCoveringPerson,
          isGoThroughWf: true,
        };
  ...
  const getRelatedEmployees = async () => {
    try {
      const actions: any = [];
      const { data } = await getEmployeeListForCoveringPerson();
  ...
  return (
    <>
      ...
      >
        <LeaveHistoryList listType="leave"></LeaveHistoryList>
      </ModalForm>
    </>
  );
};
