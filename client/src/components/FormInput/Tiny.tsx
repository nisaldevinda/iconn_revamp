...
const TinyInput: React.FC<TinyProps> = (props) => {
  ...
  const initialValue = props.values[fieldName] ?? `<p>Enter your content here.</p>`;
  ...
  const onChange = (content) => {
    ...
    currentValue[fieldName] = content;
    props.setValues(currentValue);
  }

  return (
    <Col data-key={fieldName}>
      <Row>
        <Col>
          <label title={label}>{label}</label>
        </Col>
      </Row>
      <Row style={{ paddingTop: 7, paddingBottom: 20, }}>
        <Col>
          <Editor
            apiKey={TINY_API_KEY}
